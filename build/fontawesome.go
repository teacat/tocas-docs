...
type ArticleDefinitionSectionOmitted struct {
...
func fontawesome(c *cli.Context) error {
	...
	for _, asset := range release.Assets {
		...
		if _, err := os.Stat(filepath.Join(os.TempDir(), fileprefix)); os.IsNotExist(err) {
		...
		fontsPath, err := filepath.Glob(filepath.Join(os.TempDir(), fileprefix, "webfonts", "*"))
		if err != nil {
			return err
		}
		...
		iconList, err := os.ReadFile(filepath.Join(os.TempDir(), fileprefix, "metadata", "icons.json"))
		if err != nil {
			return err
		}
		...
		//
		b, err := os.ReadFile(filepath.Join(os.TempDir(), fileprefix, "metadata", "categories.yml"))
		if err != nil {
			return err
		}
		...
		b, err = os.ReadFile(filepath.Join(os.TempDir(), fileprefix, "metadata", "icons.yml"))
		if err != nil {
			return err
		}
		var iconsx map[string]interface{}

		if err := yaml.Unmarshal(b, &iconsx); err != nil {
			return err
		}
		...
		var sorted []ArticleDefinitionSectionOmitted

		for k, v := range categories {
			final[k] = []string{}

			section := ArticleDefinitionSectionOmitted{
				Title:  k,
				Anchor: k,
			}
		...
		// write the map to a YAML file, and then save it to the Tocas UI source code.
		err = os.WriteFile(Path("{docs}/build/font_awesome.yml"), bb.Bytes(), 0777)
		if err != nil {
			return err
		}
