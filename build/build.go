...
func Path(v string) string {
	v = strings.ReplaceAll(v, "{docs}", TOCAS_DOCS)
	v = strings.ReplaceAll(v, "{src}", TOCAS_PATH+"/src")
	v = strings.ReplaceAll(v, "{dist}", TOCAS_PATH+"/dist")
	v = strings.ReplaceAll(v, "{lang}", LANG)
...
func build(c *cli.Context) error {
	...
	files, err := os.ReadDir(Path("{docs}/translations/{lang}/components"))
	if err != nil {
		return err
	}
	// create docs-dev folder for debugging in local
	err = os.MkdirAll(Path("{docs}/docs-dev/{lang}"), 0777)
	if err != nil {
		return err
	}
	// copy `/assets` from template folder to `/docs-dev`
	log.Printf("copying assets")
	if err := os.RemoveAll(Path("{docs}/docs-dev/{lang}/assets")); err != nil {
		return err
	}

	if err := exec.Command("cp", "-rf", Path("{docs}/build/templates/assets"), Path("{docs}/docs-dev/{lang}/assets")).Run(); err != nil {
		return err
	}
	log.Printf("compiling latest tocas")
	if err := compile(c); err != nil {
		return err
	}
	log.Printf("copying compiled tocas")
	if err := os.RemoveAll(Path("{docs}/docs-dev/{lang}/assets/tocas")); err != nil {
		return err
	}
	if err := exec.Command("cp", "-rf", Path("{dist}"), Path("{docs}/docs-dev/{lang}/assets/tocas")).Run(); err != nil {
		return err
	}
	if err := os.RemoveAll(Path("{docs}/docs-dev/{lang}/assets/tocas/src")); err != nil {
		return err
	}
	if err := exec.Command("cp", "-rf", Path("{src}"), Path("{docs}/docs-dev/{lang}/assets/tocas/src")).Run(); err != nil {
		return err
	}

	// 重新複製一份 `/examples` 到文件的 `/examples` 進去。
	log.Printf("copying /examples")
	if err := os.RemoveAll(Path("{docs}/docs-dev/{lang}/examples")); err != nil {
		return err
	}
	if err := exec.Command("cp", "-rf", Path("{docs}/examples"), Path("{docs}/docs-dev/{lang}/examples")).Run(); err != nil {
		return err
	}
	// 載入這個文件語系的 `meta.yml` 主要中繼檔案。
	b, err := os.ReadFile(Path("{docs}/translations/{lang}/meta.yml"))
	if err != nil {
		return err
	}
	var meta Meta
	...
	indexTmpl, err := template.New("index.html").Funcs(fmap).ParseFiles(Path("{docs}/build/templates/index.html"))
	if err != nil {
		return err
	}
	...
	log.Printf("compiled: index.html")
	...
	file, err = os.Create(Path("{docs}/docs-dev/{lang}/examples.html"))
	if err != nil {
		return err
	}
	if err = yaml.Unmarshal(b, &article); err != nil {
	...
	exampleTmpl, err := template.New("examples.html").Funcs(fmap).ParseFiles(Path("{docs}/build/templates/examples.html"))
	if err != nil {
		return err
	}
	...
	log.Printf("compiled: examples.html")

	/**
	 * [component].html
	 */

	tmpl, err := template.New("article.html").Funcs(fmap).ParseFiles(Path("{docs}/build/templates/article.html"))
	if err != nil {
		return err
	}
	...
	for _, f := range files {
		...
		func(f fs.DirEntry) {
			group.Go(func() error {
				...
				file, err := os.Create(Path("{docs}/docs-dev/{lang}/" + name + ".html"))
				if err != nil {
					return err
				}
				article := Article{
				...
				b, err := os.ReadFile(Path("{docs}/translations/{lang}/components/" + f.Name()))
				if err != nil {
					return err
				}
				if err = yaml.Unmarshal(b, &article); err != nil {
				...
				log.Printf("compiled: %s.html", name)
				return nil
			})
		}(f)
	}
...
func listMetaInformations() ([]MetaInformation, error) {
	...
	files, err := filepath.Glob(Path("{docs}/translations/*/meta.yml"))
	if err != nil {
		return infos, err
	}
...
func highlight(s string) string {
	...
	b, err := os.ReadFile(Path("{docs}/build/caches/hljs/" + hash))
	if err != nil {
		if os.IsNotExist(err) {
			os.MkdirAll(Path("{docs}/build/caches/hljs"), 0777)
		} else {
			log.Fatalln(err)
		}
	}
	...
	if err != nil {
		log.Fatalln(err.Error() + string(output))
	}
...
func beautify(s string, typ string) string {
	hash := fmt.Sprintf("%x", md5.Sum([]byte(s)))
	b, err := os.ReadFile(Path("{docs}/build/caches/beautify/" + hash))
	if err != nil {
		if os.IsNotExist(err) {
			os.MkdirAll(Path("{docs}/build/caches/beautify"), 0777)
		} else {
			log.Fatalln(err)
		}
	}
	...
	os.WriteFile(Path("{docs}/build/caches/beautify/"+hash), output, 0777)
	return string(output)
}
