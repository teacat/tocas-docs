...
func screenshot(c *cli.Context) error {
	...
	// 先建立螢幕截圖資料夾。
	if err := os.MkdirAll(Path("{docs}/examples/screenshots"), 0777); err != nil {
		return err
	}
	// 載入欲螢幕截圖的網頁資料夾。
	files, err := os.ReadDir(Path("{docs}/examples"))
	if err != nil {
		return err
	}
	// 把資料夾裡面的網頁全部照一遍。
...
func takeScreenshot(url string, isDark bool) chromedp.Tasks {
	return chromedp.Tasks{
		...
		chromedp.ActionFunc(func(ctx context.Context) error {
			...
			output, _ := os.Create(Path("{docs}/examples/screenshots/" + fmt.Sprintf("%s_%s.png", name, scheme)))
			defer output.Close()
